      ...
      >
        <div class="counter-form-phisical-activity-radios-group-radio__wrapper">
          <input
            :id="level.id"
            name="activity"
            @input="$emit('update:activityData', level.id)"
            type="radio"
            :checked="level.id === activityData"
            required
          />
          <label :for="level.id">
            {{ level.name }}
        ...
        <p
          class="counter-form-phisical-activity-radios-group-radio__description"
        >
          {{ level.description }}
        </p>
      </li>
...
export default defineComponent({
  ...
  methods: {
    changeActivity(id: string) {
      this.$emit("activityChanged", id);
    },
  },
});
</script>
