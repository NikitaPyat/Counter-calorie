<template>
    <fieldset class="form__item form__parameters" name="parameters">
            ...
            <div class="inputs-group">
              <div class="input" v-for="characteristic in characteristics" :key="characteristic">
                ...
                <div class="input__wrapper">
                  <input type="text" :id="characteristic.id" :name="characteristic.name" v-on:input="changeValue($event, characteristic.id)" placeholder="0" inputmode="decimal" maxlength="3" required>
                </div>
              </div>
            </div>
          </fieldset>
...
export default ({
        ...
        clean(value) {
          if(value) {
            this.characteristics.forEach((characteristic) => {
              document.getElementById(characteristic.id).value = null
              this.$emit(characteristic.id, 0)
            })
          }
        }
    },
  ...
  methods: {
    changeValue(event, id) {
      this.$emit(id, event.target.value)
    }
  }
})
</script>
